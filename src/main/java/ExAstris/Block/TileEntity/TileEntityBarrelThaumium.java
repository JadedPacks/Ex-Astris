...
public class TileEntityBarrelThaumium extends TileEntity implements IFluidHandler, ISidedInventory {
		...
		switch(this.getMode()) {
			...
			case BLAZE_COOKING:
				...
				if(worldObj.isRemote && worldObj.rand.nextInt(20) == 0) {
					...
					this.worldObj.spawnParticle("lava", xCoord + (worldObj.rand.nextFloat() * 0.6) + 0.2d, yCoord + 1, zCoord + (worldObj.rand.nextFloat() * 0.6) + 0.2d, 0.0d, 0.0d, 0.0d);
				}
			...
			case BLAZE:
				...
				if(!worldObj.isRemote && worldObj.difficultySetting != EnumDifficulty.PEACEFUL) {
					...
					for(int x = -1; x <= 1; x++) {
						for(int y = -1; y <= 1; y++) {
							for(int z = -1; z <= 1; z++) {
								if((worldObj.isAirBlock(xCoord + x, yCoord + y, zCoord + z) || worldObj.getBlock(xCoord + x, yCoord + y, zCoord + z) == Blocks.fire) && (worldObj.isAirBlock(xCoord + x, yCoord + y + 1, zCoord + z) || worldObj.getBlock(xCoord + x, yCoord + y, zCoord + z) == Blocks.fire) && worldObj.rand.nextInt(10) == 0 && !isDone()) {
			...
			case ENDER_COOKING:
				...
				if(worldObj.isRemote && worldObj.rand.nextInt(20) == 0) {
					...
					this.worldObj.spawnParticle("portal", xCoord + (worldObj.rand.nextFloat() * 0.6) + 0.2d, yCoord + 1, zCoord + (worldObj.rand.nextFloat() * 0.6) + 0.2d, f, f1, f2);
				}
				if(isDone()) {
			...
			case BLIZZ_COOKING:
				...
				if(worldObj.isRemote && worldObj.rand.nextInt(20) == 0) {
					...
					float f1 = (worldObj.rand.nextFloat() - 0.5F) * 0.2F;
					float f2 = (worldObj.rand.nextFloat() - 0.5F) * 0.2F;
					this.worldObj.spawnParticle("snowballpoof", xCoord + (worldObj.rand.nextFloat() * 0.6) + 0.2d, yCoord + 1, zCoord + (worldObj.rand.nextFloat() * 0.6) + 0.2d, f, f1, f2);
				}
				if(isDone()) {
	...
	public boolean canExtractItem(int slot, ItemStack item, int side) {
		if(side == 0 && slot == 0) {
			...
			return worldObj.difficultySetting.getDifficultyId() == 0 && getMode().canExtract == ExtractMode.PeacefulOnly;
		}
		return false;
	}
	...
	public boolean isItemValid(ItemStack stack) {
		...
		if(getMode() == BarrelMode.FLUID && this.isFull()) {
			...
			Fluid seedOil = FluidRegistry.getFluid("seedoil");
			return seedOil != null && fluid.getFluidID() == seedOil.getID() && Block.getBlockFromItem(item) == ENBlocks.BeeTrap;
		}
		return false;
	}
	...
	public enum BarrelMode {
		...
		BarrelMode(int v, ExtractMode extract) {
			this.value = v;
